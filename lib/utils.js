...
/** @type {string} URL of the Drush Launcher phar, used for Drupal projects. */
const DRUSH_LAUNCHER_URL = 'https://github.com/drush-ops/drush-launcher/releases/latest/download/drush.phar';
 ...
 * default mount to download when the user doesn't specify one.
 *
 * Upsun mount names are relative to the application root, not the webroot, so
 * for a project with a `web` docroot the files mount is `web/sites/default/files`,
 * not `sites/default/files`. We therefore prefix the framework's public files
 * path with the configured webroot (unless the webroot is the project root).
 *
 * @param {string} framework The `framework` recipe config value.
 * @param {string} [webroot] The `webroot` recipe config value (docroot relative to the app root).
 * @return {string} A mount path relative to the application root.
 */
exports.getFilemount = (framework, webroot = '.') => {
  const files = isWordPressy(framework) ? 'wp-content/uploads' : 'sites/default/files';
  const prefix = _.trim(webroot, './');
 ...
 *
 * For Drupal this installs the Drush Launcher rather than a specific global
 * Drush version: since Drush 9, Drush is meant to be a per-project Composer
 * dependency (a bare global `composer global require drush/drush` install
 * fails at runtime with a missing Symfony kernel class, since Drush's
 * preflight expects a site's own autoloader). The launcher is a small phar
 * that lives on PATH as `drush` and delegates to the project's own
 * `vendor/bin/drush` when present, so `lando drush ...` just works for any
 * Drupal project that already depends on drush/drush (as virtually all
 * modern ones do), and fails with a clear message otherwise.
 *
 * @param {string} framework The `framework` recipe config value.
 * @return {Array<string>} Shell commands to run during the appserver build.
 */
exports.getFrameworkBuildSteps = framework => {
  ...
  return [
    `curl -fsSL -o /tmp/drush.phar ${DRUSH_LAUNCHER_URL}`,
