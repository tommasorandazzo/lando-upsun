 ...
 * @param {string} webroot The `webroot` recipe config value, used to build the default mount.
 * @return {object} The base task configuration.
 */
const getTask = (root, framework, webroot) => ({
  service: 'appserver',
  ...
  options: {
    ...
    mount: {
      ...
      interactive: {
        ...
        default: getFilemount(framework, webroot),
        weight: 400,
      },
    },
  },
 ...
 *
 * @param {object} options `{root, framework, webroot, token}` for the current app.
...
exports.getUpsunPull = (options, tokens = []) => {
  const {root, framework, webroot, token} = options;
  return _.merge({}, getTask(root, framework, webroot), {options: {auth: getAuthOptions(token, tokens)}});
};
